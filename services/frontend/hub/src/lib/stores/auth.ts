...
import { axiosInstance, ServerResponseError } from "$lib/api/api";
...
export function setTokenData(value: TokenResponse) {
	...
	// Update the token auth headers
	axiosInstance.defaults.headers.common.Authorization = `Bearer ${value.token}`;
